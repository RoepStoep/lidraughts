...
import lidraughts.user.{ Title, User }
...
object SwissCondition {
    ...
    def apply(user: User, perf: PerfType) =
      if (user.hasTitle) Accepted
        ...
        Refused { lang =>
          ...
          trans.needNbMorePerfGames.pluralTxtTo(lang, missing, List(missing, perf.name))
        }

    def name(perf: PerfType)(implicit lang: Lang) =
      trans.moreThanNbPerfRatedGames.pluralTxt(nb, nb, perf.name)
  }
  ...
  case class MaxRating(rating: Int) extends SwissCondition {
    ...
    def name(perf: PerfType)(implicit lang: Lang) = trans.ratedLessThanInPerf.txt(rating, perf.name)
  }
  ...
  case class MinRating(rating: Int) extends SwissCondition with FlatCond {
    ...
    def name(perf: PerfType)(implicit lang: Lang) = trans.ratedMoreThanInPerf.txt(rating, perf.name)
  }
  ...
  final class Verify(historyApi: lidraughts.history.HistoryApi) {
    ...
    def apply(swiss: Swiss, user: User): Fu[All.WithVerdicts] = swiss.perfType.fold(fuccess(All.WithVerdicts(Nil))) {
      apply(_, swiss.settings.conditions, user)
    }

    def apply(perf: PerfType, all: All, user: User): Fu[All.WithVerdicts] = {
      val getMaxRating: GetMaxRating = perf => historyApi.lastWeekTopRating(user, perf)
      all.withVerdicts(perf, getMaxRating)(user)
    }

    def canEnter(user: User)(swiss: Swiss): Fu[Boolean] =
      swiss.perfType ?? { perf =>
  ...
  object BSONHandlers {
    ...
    implicit private val TitledHandler = new BSONHandler[BSONValue, Titled.type] {
