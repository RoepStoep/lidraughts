...
def lint_string(ctx, dest, source, allow_missing=0):
    ...
    if "lishogi" in dest.lower() and not "lishogi" in source.lower():
        ctx.error("lishogi in dest while not in source")

    if "lidraughts" in source.lower() and "lidraughts" not in dest.lower():
        ctx.warning("missing lidraughts")
